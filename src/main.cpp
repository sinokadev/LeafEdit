...
std::string GetLinuxSystemMonospaceFontPath() {
    ...
    // [핵심] 특정 폰트 이름 대신 "monospace"를 주어 시스템 고정폭 대표 서체를
    // 요청합니다.
    ...
    // FcFini(); // 프로그램 종료 시에만 호출하는 것이 안전하므로 여기선 생략
    // 가능
...
void ConfigureLinuxNativeFont(ImFontConfig &config) {
    ...
    if (match) {
        ...
        if (antialias == FcFalse) {
            ...
            // [수정] 웹 브라우저 느낌을 내기 위해 힌팅 강도를 한 단계씩 낮추어
            // 매핑합니다.
            if (hinting == FcFalse || hint_style == FC_HINT_NONE) {
                ...
                // Medium 힌팅까지는 브라우저처럼 부드러운 LightHinting으로
                // 처리하는 것이 정신건강에 좋습니다.
    ...
    while (!done) {
            ...
            if (event.type == SDL_EVENT_KEY_DOWN) {
                if (event.key.key == SDLK_BACKSPACE) {
                    ...
                    if (m_CursorByteOffset > 0) {
                        // [핵심] 현재 위치 바로 앞의 글자 바이트 길이를
                        // 역방향으로 탐색
                        int bytes_to_remove = 1; // 기본값

                        // 뒤에서부터 1~4바이트를 조사하여 UTF-8 시작
                        // 바이트(10xxxxxx가 아닌 것)를 찾음
                        int i = 1;
...

        {
            ...
            float total_line_height =
                ImGui::GetTextLineHeight() + 6.0f; // 줄높이 + 행간 마진
            float char_width =
                current_font
            ...
            for (size_t i = 0; i < m_Lines.size(); i++) {
                ...
                float text_draw_y =
                    box_top_y + 3.0f; // 마진 절반 내려서 정중앙 정렬
                ...
                if (i == (size_t)m_CursorLine) {
                    ...
                    // 간단한 보정: 3바이트 한글의 중간을 가리키고 있다면
                    // 앞/뒤로 강제 이동 (물론 문자 단위로 인덱스를 관리하는
                    // 것이 최선입니다)
                    ...
                    // 2단계: OS가 조합 중인 한글이 있다면 중간에 끼워 넣어서
                    // 그리기
                    if (!m_CompositionText.empty()) {
                        ...
                        // 브라우저 느낌을 내기 위해 조합 중인 글자 아래에 빨간
                        // 밑줄을 그어줍니다.
                        draw_list->AddLine(
                            ImVec2(current_x,
                        ...
                        current_x += comp_width; // 가상 커서 위치를 조합 문자의
                                                 // 끝으로 이동
                    } else {
                        // 조합 중인 글자가 없을 때만 진짜 깜빡이는 바(Bar)
                        // 커서를 그립니다.
